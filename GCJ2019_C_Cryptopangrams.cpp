...
{
    Letters() {}
    Letters(const LL &mult_) : p1(0), p2(0), value(mult_), l1('_'), l2('_') {}
...
public:
    ...
    void solve(int problemIdx)
    {
        ...
        char letter = 'A';
        map<LL, char>    primeToLetter;
        for (int prime : primes)
        {
            for (auto &pair : input)
            {
                LL mod = pair.value % prime;
                if (mod == zero)
                {
                    pair.p1 = prime;
                    pair.p2 = pair.value / prime;

                    if (primeToLetter.find(prime) == primeToLetter.end())
                    {
                        primeToLetter[prime] = letter++;
                    }
                }
            }
        }
        ...
        result.clear();
        result.push_back(primeToLetter[input[0].p1]);
        result.push_back(primeToLetter[input[0].p2]);
        for (size_t i=1; i<input.size(); i++)
        {
            result.push_back(primeToLetter[input[i].p2]);
        }
    }
